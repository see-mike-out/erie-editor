...
<section id="background" aria-roledescription="Background and Related Work">
  ...
  {#if !collapse}
    ...
    <p aria-roledescription="paragraph">
      ...
      descriptions. Yet, accessibility is not the only venue for sonification,
      but various fields, such as scientific data representation
      <Cite
        content="andrea2005:meteorology,john1999:lifemusic,ghosh2010:particle"
    ...
    <p aria-roledescription="paragraph">
      ...
      Different auditory channels, such as pitch or volume, are physicalized into
      a waveform. We first describe a few core concepts related to a sound wave:
      <em>frequency</em>
      ...
      cycles, and people tend to perceive it as a higher pitch. The amplitude of
      a sound wave means the extent or depth of a waveform. A larger amplitude
    ...
    <p aria-roledescription="paragraph">
      ...
      modulator that changes the carrier's waveform through some signal
      processing (simply put). A <em>modulation index</em> (MI) for such synths
      refers to the degree of modulation in signal processing. The frequencies
      of two oscillators generate the <em>harmonicity</em> between them.
    </p>
    ...
    <p aria-roledescription="paragraph">
      ...
      audio scale, audio scales are capped by the physical space. For example,
      the common audible frequency spectrum is known to range from 20 Hz to
      20,000 Hz <Cite content="audibleSpectrum"></Cite>.
    </p>

    <p aria-roledescription="paragraph">
      <strong>Empirical studies in data sonification for accessibility</strong>
      focus on how people with BVI interpret different auditory mappings. Walker
      et al. <Cite
      ...
      (e.g., dollars, temperature). Recent work extends focus to other qualities
      of auditory mappings. For instance, Hoque et al. <Cite
      ...
      quantitative, ordinal). In their experiment, participants indicated a need
      for an overview of auditory scales <Cite content="wang2022:intuitiveness"
    ...
    <p aria-roledescription="paragraph">
      ...
      ></Cite>, a geospatial data analytic tool, offers various audio feedback
      for browsing maps, such as using stereo panning to provide a spatial sense
      of the geospatial data point that a user is browsing. iGraph-Lite <Cite
    ...
    <p aria-roledescription="paragraph">
      ...
      support for creating expressive and data-driven sonifications as summarized
      in
      ...
      channels. Sonifier.js <Cite content="sonifier,sharif2022:sonifier"></Cite>
      offers limited audio channels, time and pitch. Sonification Sandbox
      ...
      existing tools lack application programming interface (API) support, making
      it difficult for users to personalize or customize sonification designs with
      their preferred encoding channels or instruments. To achieve greater expressiveness
      with APIs, developers could use audio programming libraries, such as Tone.js
      <Cite content="tonejs"></Cite>, but they have to manually scale data
      values to auditory values, which can be a substantial hurdle for those
      with limited audio skills. These tools also lack support for scale
      references (e.g., tick, scale description), making it harder to decode
      audio graphs they generate.
    </p>

    <p aria-roledescription="paragraph">
      Our work provides a declarative grammar for data sonification, <em
        >Erie</em
      ...
      loudness, tapping, panning, reverb, and speech), verbal descriptions, tone
      sampling, and composition methods (repeat, sequence, and overlay), making it
      a good basis for use in the development of future sonification software.
    </p>
    ...
    <p aria-roledescription="paragraph">
      ...
      provides an abstract specification (or spec) describing the intended
      outcome and a compiler executes to generate the outcome. In this paradigm,
      declarative grammar defines rules for how to write a program. Many
      data-related fields, such as data visualization and statistics, have
      widely adopted declarative grammars. In data visualization, Wilkinson <Cite
    ...
    <p aria-roledescription="paragraph">
      ...
      compilers for user applications, particularly when directly manipulating
      the targeted physical space is challenging like audio environments for
      sonification <Cite content="joyner2022:challenge"></Cite>. For example,
      some sonification toolkits (e.g., <Cite content="istk:pauletto"></Cite>)
      adopt visual programming languages to allow for visually and interactively
      authoring data sonification, and those visual programming languages are
      backed by some kind of declarative expressions. For example, Quick and
      Hudak <Cite content="donya:haskell2013"></Cite> provide graph-based expressions
      that allow for specifying constraints to automatically generate music. Implementing
      a sonification from scratch requires a sophisticated skill set for controlling
