...
<section id="grammar" aria-roledescription="Erie Grammar">
  ...
  {#if !collapse}
    <p aria-roledescription="paragraph">
      ...
      <strong>data-driven (C3)</strong>. At a high level, <em>Erie</em>
      expresses a sonification design using a sound instrument (<em>tone</em>)
      and mappings from data to auditory values (<em>encoding</em> channels).
      After walking through an example case, we describe how <em>Erie</em>
      expresses a data sonification design, including top-level specification,
      stream, data input and transform, tone, encoding, stream composition, and
      configuration. The formal definition of <em>Erie</em> is provided in <Ref
        key="fig-grammar-formal"
      ></Ref>. In describing <em>Erie</em>, we distinguish <em>developer</em>s
      who create sonifications from <em>listener</em>s who listen to
      sonifications. For details, refer to the <Ref key="appendix"></Ref> and
      <a
    ...
    <p aria-roledescription="paragraph">
      To help imagine how <em>Erie</em> works in specifying a sonification
      design, we introduce a simple auditory histogram for a quantitative data
      variable,
    ...
    <p aria-roledescription="paragraph">
      To construct this example using <em>Erie</em>, we first specify the data
      to sonify by providing its URL:
      <Math
 ...
 \\textit{auto}=\\textit{\\textcolor{gray}{true}}, 
  ...
  \\textit{end}=\\textit{\\textcolor{gray}{miles-per-gallon-bin-end}} \\}`}
      ...
      <em>count</em>. To count the values for each bucket, this expression sets
      a
      <em>group-by</em>
      field to the bin start point field (<em>miles-per-gallon-bin</em>)
      generated by the previous bin transform.
      <Math
      ...
      <Math
        content={`\\textit{transform} = [\\textit{bin}, \\textit{aggregate}]`}
      ></Math>
    ...
    <p aria-roledescription="paragraph">
      ...
      ></Math>
      The above expression also specifies that the <em>time</em> channel encodes
      a
      <em>quantitative</em>
      ...
      because it uses the same scale as the <em>time</em> channel. Next, we
      encode the
      <em>count</em>
    ...
    <p aria-roledescription="paragraph">
      ...
      represents a unit sonification design, consisting of <em>data</em> (what
      to sonify), <em>transform</em> (operations to the data), <em>tone</em>
      (overall sound quality), and <em>encoding</em> (mappings from data to
      sound values):
      <Math
      ...
      <em>tick</em>,
      <em>synth</em>
      (synthesizers), <em>wave</em> (periodic wave), and <em>sampling</em>
      (using external audio files as a <em>tone</em>). A developer can specify a
      speech-based <em>title</em> and <em>description</em> that are played
      before the audio graph. The <em>config</em> of a <em>spec</em> configures
      a sonification design, such as the speed of speech (<em>speech rate</em>)
      and whether to skip playing the <em>title</em> text (<em>skip title</em>).
    </p>
    ...
    <p aria-roledescription="paragraph">
      ...
      supports three methods to do so: providing the <em>URL</em> of a data
      file, providing an array of <em>values</em>, or providing the
      <em>name</em>
      of a predefined dataset in the <em>dataset</em> object.
      <Math
        content={`\\textit{data} := \\textit{URL}~|~\\textit{values}~|~\\textit{name},`}
      ></Math>
      where the vertical bar sign <Eq math={`|`}></Eq> denotes 'or'. A
      <em>dataset</em>
      object consists of the named definitions of data items using <em>URL</em>
      or
      <em>values</em>.
      <Math
      ...
      <em>transform</em>
      object. In the walkthrough, for example, the <em>bin</em> transform
      created new data variables for the start and end points of each bin, and
      the
      <em>count</em> <em>aggregate</em> reshaped the data with a new variable for
    ...
    <p aria-roledescription="paragraph">
      ...
      glyph in a visualization given that data values are mapped to its
      properties like pitch.
      ...
      ></Math>
      An instrument <em>type</em> can be specified by its name, such as
      'sawtooth' (oscillator) or 'violin', where default is a sinusoidal
      oscillator in our implementation. If a sound is <em>continued</em>, two
      sound points are connected without a pause. For more diverse audio
      expressions, the developer can provide audio <em>filter</em>s like
      distortion or equalizer.
    </p>
    ...
    <p aria-roledescription="paragraph">
      ...
      different auditory properties (e.g., pitch and loudness) of a
      <em>tone</em>.
      <em>Erie</em> supports three classes of channels: dynamic, conditioned,
      and static. A <em>d</em>ynamic <em>channel</em> encodes a data variable
      (or field) to the respective auditory property. It is defined in terms of
      a data
      <em>field</em>
      from the <em>stream</em>'s data, the data type of an encoding (<em
        >enc-type</em
      >), its <em>scale</em> details, its <em>ramp</em>ing method, and inline
      data transform options (<em>aggregate</em> and <em>bin</em>):
      <Math
    ...
    <p aria-roledescription="paragraph">
      The<em>scale</em> of a dynamic encoding channel essentially consists of
      the
      <em>domain</em>
      ...
      function
      <Eq math={`f(x) = (660-220) \\times \\frac{x}{100} + 220`}></Eq>. There
      are shortcuts for defining a <em>range</em>. When <em>max-distinct</em> is
      set to
      <em>true</em>, then the widest possible range is used (e.g., the lowest to
      highest human-audible pitch). The <em>times</em> multiplies each data
      value by itself to compute auditory values. To verbally describe the
      scale, a developer can provide <em>description</em> using a markup
      expression (see <Ref key="appendix-encoding-reference"></Ref>), analogous
      to a legend in a visualization. A baseline
      <em>scale</em>
    ...
    <p aria-roledescription="paragraph">
      Combining multiple<em>stream</em>s is necessary to create rich auditory
      data narratives (e.g., <Cite
      ...
      region. Streams for different polls can be overlaid to support
    ...
    <p aria-roledescription="paragraph">
      ...
      walkthrough case by replicating the auditory histogram by the<em
        >origin</em
      >
      ...
      <em>stream</em>
      design by one or more data fields. From the previous example, the developer
      can repeat the auditory histogram by the <em>origin</em> and the number of
      <em>cylinders</em>
      (values: 3, 4, 5, 6, and 8):
      <Math
        content={`\\textit{repeat} = \\{ \\textit{field} = \textcolor{gray}{[ \\textit{origin}, \\textit{cylinders} ]} \\}`}
      ></Math>
      In this case, the repeat order is nested, such that the histograms for the
      cylinder values are played for each origin. A <em>repeat</em> channel has
      a
      <em>speech</em>
    ...
    <p aria-roledescription="paragraph">
      ...
      auditory histograms for different cylinder values to reduce the playtime.
      To do so, the developer can use the <em>by</em> property in the
      <em>repeat</em>
      channel:
      <Math
      ...
      ></Math>
      This results in a sonification queue shown in <Ref
        key="tab-grammar-repeat2"
      ></Ref>.
    </p>
    ...
    <p aria-roledescription="paragraph">
      Two or more separate<em>stream</em>s can be combined as a
      <em>sequence</em>
      (playing one after another) or an <em>overlay</em> (playing all together
      at the same time). To enable these multi-stream compositions, we extend
      ...
      <em>config</em>
      object in a sub-<em>stream</em> overrides the top-level <em>config</em>.
      The
      <em>transform</em>
...
\\textit{sequence} := [\\textit{stream} ~|~ \\textit{overlay} ]`}
      ></Math>
      Note that a nested sequence, <Eq math={`[{sequence}, {sequence}]`}></Eq>,
      is also a <em>sequence</em>.
    </p>

    <h3 aria-roledescription="subsection title">
      <a
        class="anchor"
        name="grammar-configuration"
        href="#grammar-configuration">5.7. Configuration</a
      >
    </h3>
    <p aria-roledescription="paragraph">
      A<em>config</em> object specifies overall controls for the sonification.
      For example, when a sonification consists of multiple streams that use the
      same auditory encodings and scales, the developer can skip playing the
      scale descriptions for the non-first streams. When a sonification needs
      more musical representation, a developer can change the <em>time-unit</em>
      from seconds (default) to beats. For background, when BPM is 100, one beat
      corresponds to 0.6 seconds (<Eq math={`= 60/100`}></Eq>). In this case,
      the developer can specify the tempo (beat per minute, or BPM) and whether
      to round raw beats to integer beats (e.g., 3.224 to 3). When the time unit
      of sonification is set to beats, then other time-related units are also
