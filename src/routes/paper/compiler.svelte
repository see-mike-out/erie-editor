...
<section
  id="compiler"
  aria-roledescription="Erie Compiler and Player for Web"
...
>
  ...
  <p aria-roledescription="paragraph">
    ...
    compiles a spec to an <em>audio queue</em> representing a schedule of sounds
    to be played in terms of their physical values. Then, a <em>player</em>
    renders this audio queue into actual sounds. We separate the queue compiler from
    the player to allow listeners to control when to play or pause a sonification
    and to support developing players for different audio environments, such as CSound
    <Cite content="csound"></Cite>. We implemented and open-sourced a spec API,
    a queue compiler, and <a href="https://github.com/see-mike-out/erie-web" target="_blank">a player for a web environment</a> using web standard APIs in JavaScript (<strong
  ...
  <p aria-roledescription="paragraph">
    ...
    include aggregation, binning, kernel density estimation, folding (columns to
    rows; e.g., <Eq
  ...
  <p aria-roledescription="paragraph">
    ...
    pink, and brown), simple oscillators (sine, sawtooth, triangle, and square forms),
    configurable FM and AM synths, and periodic waves.
  </p>
  <p aria-roledescription="paragraph">
  ...
  <p aria-roledescription="paragraph">
    ...
    Our queue compiler handles <em>time</em>, <em>time2</em>, <em>duration</em>,
    ...
    <em>speech-after</em>, and <em>repeat</em> channels. Different audio filters
    can have extra encoding channels. For example, a lowpass biquad filter
    attenuates frequencies above a certain cutoff, and it can have a
    <em>biquad-frequency</em>
    channel to set the cutoff.
  </p>
  <p aria-roledescription="paragraph">
    <strong>Scale descriptions.</strong>
    <em>Erie</em>'s queue compiler generates a description of each scale to give
    an overview of the sonification. A scale description functions as an
  ...
  <p aria-roledescription="paragraph">
    We implemented<em>Erie</em> syntax in JavaScript. For example, the spec of the
    walkthrough can be written as below.
  </p>
...
spec.encoding.pitch.field("count", "quantitative")
  ...
  <p aria-roledescription="paragraph">
    This spec is equivalent to the following JSON object, which can be obtained
    via the <code>get</code> method of the spec API. This JSON syntax reuses
    some Vega-Lite <Cite content="satyanarayan:vega-lite2017"></Cite> expressions,
...
{ "data": { "url": "cars.json" },
  ...
  <p aria-roledescription="paragraph">
    ...
    The outcome audio queue is an ordered list of sub-queues, and each sub-queue
    item can have one of these four types:<em>speech</em>, <em>tone-series</em>,
    ...
    queue is a timed list of sounds and speeches. Each sound in a sub-queue of these
    two types is expressed in terms of their actual auditory values (e.g., Hz for
    pitch). Lastly, a
    <em>tone-overlay</em>
  ...
  <p aria-roledescription="paragraph">
    We developed an<em>Erie</em> player for web environments using the standard
    Web Audio API <Cite content="webAudio"></Cite> and Web Speech API <Cite
      content="webSpeech"
    ></Cite>. The player offers several playing options: play from the
    beginning, pause, resume, stop, play from a sub-queue, and play from one
...
audioQueue.queue.play(i, j); // Play the i-th to (j-1)-th sub-queues.`}
  ...
  <p aria-roledescription="paragraph">
    ...
    Furthermore, those audio filters can encode data variables (e.g., the amount
    of distortion to express air quality). To widen such design possibilities,<em
      >Erie</em
    >
    ...
    defining a custom filter, imagine that a developer wants to add an envelope
    filter with encodable <code>attack</code> and <code>release</code>
    times.
    <code>Attack</code> means the time duration from the zero volume at the
    beginning of a sound to the highest volume, and <code>release</code> refers
    to the time taken from the highest volume to the zero volume at the end of
    the sound <Cite content="appleEnvelope"></Cite>. The developer first needs
    to define the filter as a JavaScript <code>class</code> that can be chained
    from a sonification sound to an output audio device. This class should have
    <code>connect</code>
    ...
    developer needs to define an <code>encoder</code> function that assigns the
    <code>attack</code>
    value for each data value to the filter and a <code>finisher</code> function
    that assigns the <code>release</code> values to the filter. Refer to the documentation
    in our Supplementary Material for technical details.
  </p>
</section>
