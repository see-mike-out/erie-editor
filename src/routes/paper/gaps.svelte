...
>
  ...
  {#if !collapse}
    <p aria-roledescription="paragraph">
      To motivate our design of <em>Erie</em> with awareness of existing
      practices used in developing data sonification, we surveyed recently
      published data sonification tutorials and designs. To understand practices
      being shared among sonification developers, we collected nine online
      tutorials for coding sonifications by searching with keywords like
      "sonification tutorial," "audio graph tutorial," or "sonification code."
      To see techniques beyond tutorials, we inspected 24 data sonifications
      with code or detailed methodology descriptions from
      ...
      and
      <a href="https://www.loudnumbers.net/" target="_blank">Loud Numbers</a>.
    ...
    <p aria-roledescription="paragraph">
      ...
      >
      Most tutorials (7 out of 9) introduced
      <em>music programming libraries</em>
      like music21, PyGame, Tone.js, sequenceR, Max, Sonic Pi, and MIDIFile, and
      most (15 out of 24) sonification designs used them. These libraries take as
      input auditory values like pitch notes or frequencies, volumes, and time durations.
      That is, developers still need to define scale functions that convert data
      values to auditory values, requiring an understanding of physical properties
      of different auditory variables. For example, the
      <a
      ...
      by a scalar to keep them in a certain range. Third, the developer specifies
      a list of notes to map data points to. Last, they write a for loop to convert
      each data point to the corresponding note from the list. On the other hand,
      <a
    ...
    <p aria-roledescription="paragraph">
      ...
      and distortion level are governed by different audio nodes. More
      experienced professional creators chose to use more advanced music
      software like Ableton Live, Supercollider, and Touch Designer that enable
      live performances or art installations.
    </p>
    <p aria-roledescription="paragraph">
      ...
      designs. For example, to change the domain, range, and transformation type
      (e.g.,sqrt, log) of a certain scale, then they have to find the relevant lines
      and manually change them by writing something like a linear scale function
      (e.g.,
      <code
      ...
      widely known among visualization practitioners<Cite
        content="battle2022:d3"
      ></Cite>.
    </p>
  {/if}
</section>
