...
<section id="appendix" aria-roledescription="Technical Details of Erie">
  ...
  {#if !collapse}
    ...
    <p aria-roledescription="paragraph">
      ...
      instrument <em>type</em>, whether the sound is <em>continued</em>, and a
      set of audio <em>filter</em>s. To use custom instruments to express
      diverse sonification designs, a developer can define new instruments using
      <em>synth</em>
      (for FM and AM synthesizer), <em>wave</em> (directly defining a wave
      function), and <em>sampling</em> (importing external audio files) objects
      in a top-level spec. The developer can apply custom instruments to the
      <em>tone</em> <em>type</em> and a <em>timbre</em> encoding channel by referencing
      their names.
    </p>

    <p aria-roledescription="paragraph">
      A dataset typically exists as a set of data points; even if it represents
      a continuous distribution, its digital format is a set of approximated
      data points. Thus, a data representation should be able to capture the
      continuity or discreteness between data points (e.g., line chart vs.
      scatterplot). In the walkthrough, for example, we used a <em>discrete</em>
      (<Eq math={`\\textit{continued} = false`}></Eq>) tone to indicate that
      each sound represents a discrete bin. On the other hand, a developer could
    ...
    <p aria-roledescription="paragraph">
      When more artistic sound effects (e.g., dynamic compression, distortion)
      are needed, a developer can apply them using the <em>filter</em> property
      of a
      <em>tone</em>. A <em>filter</em> object is an ordered list of filter
      names, and each filter is applied after the previous filter, reflecting
      how audio filters are commonly applied to electric sounds.
    ...
    <p aria-roledescription="paragraph">
      Below, we detail how to indicate specific properties for different
      encoding channels and auxiliary or shortcut properties for diverse
    ...
    <p aria-roledescription="paragraph">
      ...
      one hand, two data variables sharing the same unit (e.g., monthly lowest
      and highest temperature) can be mapped to start and end times. On the
      other hand, two data variables with different units (e.g., country names
      and CO2 emissions) can be mapped to start time and duration.
      <em>Erie</em> supports expressing when a sound starts and ends (<em
        >time</em
      >
    ...
    <p aria-roledescription="paragraph">
      The length of a sonification is also the <em>range</em> of its time
      channel. Thus, <em>Erie</em> provides another shortcut, <em>length</em>,
      for the
      <em>range</em>
    ...
    <p aria-roledescription="paragraph">
      <em>Erie</em> makes a distinction between <em>when a sound starts</em>
      (the value of the <em>time</em> channel) and <em>how a sound is timed</em>
      in relation to other sounds (<em>timing</em>). For example, a developer
      wants a sound to be played after the previous sound (<em>relative</em>),
      to start on an exact time (<em>absolute</em>), or to start with the other
      sounds (<em>simultaneous</em>). To control how a <em>time</em> channel
      assigns times, the developer can use the <em>timing</em> property of the
      <em>time</em>
      channel's scale. The above extensions to the <em>time</em> channel's scale
      is formalized as:
      <Math
    ...
    <p aria-roledescription="paragraph">
      ...
      defines sounds with a fixed duration (<Eq math={`n`}></Eq>) and start
      times varied by the encoded data variable (<Eq math={`x`}></Eq>). If
      <em>scale.timing</em>
    ...
    + \\textit{time2}(\\textit{field}=y)`}
      ...
      sounds with varying start and end times. Note that the two fields mapped
      to the <em>time</em> and <em>time2</em> channel must be defined on the same
      data unit, such as bin start and endpoints.
    </p>
    ...
    <p aria-roledescription="paragraph">
      Specific auditory encoding channels may have different physical
      constraints that need channel-specific properties in addition to the above
      definition.
      <em>Erie</em> considers such physical constraints in defining encodings
      for canonical auditory channels. For example, <em>pitch</em> can have raw
      pitch frequency values or have them rounded to musical notes. To enable
      this rounding, a developer can set <em>round-to-note</em> to <em>true</em>
      for the <em>pitch</em> channel.
    </p>
    ...
    <p aria-roledescription="paragraph">
      <em>Tick for time channel</em>. A longer sonification may need to provide
      a sense of the progression of time as Cartesian plots have axis ticks and
    ...
    <p aria-roledescription="paragraph">
      ...
      employs a markup expression that can express literal texts, audio legends (<em
        ><sound></sound></em
      >), a list of items (<em><list></list></em>), and reserved keywords, such
      as
    ...
    <p aria-roledescription="paragraph">
      Inspired by Vega-Lite~<Cite content="satyanarayan:vega-lite2017"></Cite>,
      it is possible to provide an inline data transform: <em>aggregate</em> or
