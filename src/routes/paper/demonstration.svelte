...
<section id="demonstration" aria-roledescription="Demonstration">
  ...
  {#if !collapse}
    <p aria-roledescription="paragraph">
      ...
      and <strong>expressiveness (C2)</strong>, we walk through novel examples.
      We also replicated and extended prior sonifications to show the
    ...
    <h3 aria-roledescription="subsection title">
      <a
        class="anchor"
        name="demonstration-example"
        href="#demonstration-example">7.1. Example Sonification Designs</a
      >
    </h3>

    <p aria-roledescription="paragraph">
      We show three representative example cases to show how <em>Erie</em> can be
      used.
    </p>
    ...
    <div class="code-paragraph">
      <div class="paragraph">
        <p aria-roledescription="paragraph">
          ...
          tap-speed channel. We have a nominal variable, dataset <code
            >name</code
        ...
        <p aria-roledescription="paragraph">
          Now, we want to map the <code>name</code> field to the
          <code>time</code>
          ...
          tapping. First, we create a single-stream sonification spec object and
          set a description text (<LineNo>7–9</LineNo>). Then, we assign the
          <code>data</code>
          to this spec (<LineNo>9</LineNo>). With a default sine-wave
          oscillator, we need a discrete tone to represent separate data tables,
          which can be specified as <LineNo>10</LineNo>.
        </p>

        <p aria-roledescription="paragraph">
          Next, we set the <code>time</code> encoding channel as described
          earlier (<LineNo>11</LineNo>). This <code>time</code> channel should
          use relative timing to allow for playing each data table name before
          the sound for the corresponding sparsity value (<LineNo>12</LineNo>).
          We then specify the <code>tapSpeed</code> channel for the quantitative
          sparsity channel (<LineNo>13</LineNo>). This <code>tapSpeed</code>
          channel has the domain of <Eq math="[0, 1]"></Eq>. We want to map this
          domain to the range of <Eq math="[0, 5]"></Eq> (i.e., zero to five taps
          per second) for 2 seconds (<LineNo>14</LineNo>). Since a higher
          sparsity value should have a lower speed, we need negative
          ...
          sound in the middle of the time <code>band</code> (two seconds), we
          set the
          ...
          <Ref key="tab-demo-sparsity"></Ref> shows the audio queue compiled from
          this spec.
        </p>
      </div>
      <div class="code">
...
spec.encoding.speechBefore.scale("description", "skip");`}
    ...
    <p aria-roledescription="paragraph">
      ...
      variables of interest is a common first step. It is important to observe
      the entirety of a distribution because some distributional information,
      such as multi-modality, are not captured by summary statistics like mean
      and variance. In addition to histograms, data analysts often estimate the
      probability density of a quantitative variable using a kernel smoothing
      function (i.e., kernel density estimation or KDE). In this example, we
      want to sonify a KDE of the <code>bodyMass</code> variable of the
      <code>penguins.json</code>
      data <Cite content="vegadataset"></Cite>. This sonification will encode
      the density by pitch and the variable's value by time and panning. Then,
      we repeat this sonification design for different <code>species</code> and
    ...
    <p aria-roledescription="paragraph">
      ...
      <code>species</code>
      and <code>island</code> fields form five combinations as shown in the
      first two columns of <Ref key="tab-demo-kde-data"></Ref>. The
    ...
    <div class="code-paragraph">
      <div class="paragraph">
        <p aria-roledescription="paragraph">
          ...
          <code>value</code>
          element), <code>species</code>, and <code>island</code>. Third, we use
          a
          <code>continued</code>
        ...
        <p aria-roledescription="paragraph">
          ...
          and spatial senses of sound progression (<LineNo>8–9</LineNo>). Then,
          we detail the <code>scale</code> of the <code>time</code> channel by
          setting the <code>length</code> of each repeated sound to three
          seconds and indicating the <code>title</code> of this
          <code>scale</code>
          in the scale description (<LineNo>10</LineNo>). Similarly, we set the
          <code>polarity</code>
          ...
          decimals (e.g., <Eq math={`0.0124 \\ldots`}></Eq>), which makes it
...
spec.encoding.repeat.field(["species", "island"], "nominal")
    ...
    <p aria-roledescription="paragraph">
      ...
      (independent variable) and <code>residuals</code>. For the QQ plot, we use
      a
      <code>qq</code>
    ...
    <div class="code-paragraph">
      <div class="paragraph">
        <p aria-roledescription="paragraph">
          We first register the datasets (<LineNo>1–6</LineNo>). Second, we
          define a sonification for a residual plot. When errors of a model fit
          are unbiased, the residuals are evenly distributed along values of the
          independent variable and on both sides of the central line indicating
          0 error. With this residual plot sonification, we want to capture the
          ...
          mapped to a more warped sound, and a negative residual is played on
          the left side and a positive residual is played on the right side. A
          good model fit will generate a sonification where the sound quickly
          (e.g., 150 sound points within 5 seconds) moves between different
          modulation index and pan values, making it harder to differentiate
          their degrees of warping and spatial positions. In contrast, a bad
          model fit will generate an audio graph where listeners can easily
          sense some groups of sounds sharing the same degree of warping on a
          certain spatial position. We use a <code>time</code> channel for the
          <code>sepalLength</code> field.
        </p>

        <p aria-roledescription="paragraph">
          To do so, we create a single stream with the <code>residuals</code>
          dataset (<LineNo>7–9</LineNo>). For the tone, we use an FM synth,
          named
          <code>fm1</code>
        ...
        <p aria-roledescription="paragraph">
          ...
          exhibit normally distributed residuals. By plotting the quantiles of
          the residuals against the expected quantiles of a normal distribution
          (range from 0 to 1), we want to observe how much the residuals deviate
          from the expectation that they are normallly distributed. A visual QQ
          plot shows the gap between the theoretical and observed distribution
          by plotting them in a Cartesian space, which is the same format used
          for a residual plot at high level. However, a sonification author may
          need to directly encode the gap because overlaying the normal and
          residual distributions with different pitches or volumes may make it
          harder to decode the gap, indicating the need for specifying a
          sonification design <strong
        ...
        <p aria-roledescription="paragraph">
          ...
          left or right when the signed deviation is bigger (<LineNo
            >26–33</LineNo
          >).
        </p>
...
modelFit.description("The residuals of a linear regression model of Sepal Length on Petal Length.");`}
    ...
    <p aria-roledescription="paragraph">
      ...
      each segment. To show how <em>Erie</em> can be used in such applications
      to generate sonifications, we created an example case that Audio Narrative
      could create by using <em>Erie</em> for sonification and speech
      generation, as shown in <Ref key="fig-demo-narrative"></Ref>. We used a
      <em>'stocks.json'</em>
    ...
    <p aria-roledescription="paragraph">
      ...
      application can simply write a sonification spec for each segment as
      below:
      <CodeViewer
        autoHeight={true}
  ...
  "encoding": {
    ...
    "pitch": { "field": "stock price", ... } } }`}
      ...
      for each segment. Having the above as a template, the application then
      merge the specs for all the segments as a <code>sequence</code>:
      <CodeViewer
        autoHeight={true}
  ...
  "config": { 
    ...
    <p aria-roledescription="paragraph">
      Given a visualization, Chart Reader <Cite
        content="thompson2023:chartreader"
      ></Cite> enables hover interaction that reads out values and generates a sonification
      for the selected data mark(s). Furthermore, Chart Reader supports creating
      several "data insights" that allow a sonification author to draft more customized
      text messages, similar to the chart segments supported by Audio Narrative.
      We replicate this use case by reusing the above Audio Narrative replication,
      given their underlying structural similarity (segmentation of a chart with
      descriptive text), as shown in <Ref key="fig-demo-reader"></Ref>.
    </p>
    ...
    <p aria-roledescription="paragraph">
      ...
      same segment template spec as Audio Narrative replication, but they are
      not sequenced. We set the pitch scale's <code>domain</code> as the minimum
      and maximum values of the sonified variable so that the segments can share
      the same pitch scale even though they are not sequenced in the same specification.
      This technique is often used in data visualization cases as well. We further
      include several customization options for toggling the hover interaction and
      data aggregation. By reusing the above sequence, we also include an option
      to play all the `data insight' segments.
    </p>
    ...
    <p aria-roledescription="paragraph">
      ...
      shooting case in Orlando in 2016 and the use of automatic weapons. In this
      article, a dot plot visualizes the shooting count over time to show how fast
      shots were fired. To make it even more realistic, the authors of this article
      included a sonification that mimics actual gun sounds.
    </p>
    <p aria-roledescription="paragraph">
      We replicate this news article sonification by mapping the shooting time
      to a <code>time</code> channel and the three cases (Las Vegas, Orlando,
      and automatic weapon) to a <code>repeat</code> channel, as shown in <Ref
        key="fig-demo-gunman"
      ...
      gunshot sound. The original article had an interaction that when the name
      of a case is selected, it plays only the relevant part. To support that,
      we use the <code>audioQueue.play(i, j)</code> method, so that the player
      only plays from the <code>i</code>-th sub-queue to <code>j</code>-th
      sub-queue. In this case, the first sub-queue is the name of a case, and
      the last sub-queue is the sonification sound (two sub-queues in total).
    </p>
