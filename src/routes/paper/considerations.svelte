...
>
  ...
  {#if !collapse}
    ...
    <p aria-roledescription="paragraph">
      ...
      Many existing sonification libraries that provide APIs are strongly tied to
      visual forms, such that they support sonifying a particular visualization instead
      of authoring a sonification. While this approach can make it easy to generate
      sounds, it prevents sonification creators from exploring the many alternative
      designs one might generate by directly expressing audio graphs. Furthermore,
      it ignores different tasks implied by similar visualization designs. For example,
      point marks can be a scatterplot for assessing correlation or a residual plot
      for judging model fit, potentially calling for different sonification designs.
      We designed the <em>Erie</em> grammar to be independent of visual forms to
      maximize design possibilities.
    </p>
    <p aria-roledescription="paragraph">
      <strong>(C2) Be expressive.</strong>
      To support independently creating various sonification designs, it must be
      possible to express different sound qualities, auditory channels, and combinations
      of multiple sonifications. Expressive toolkits enable researchers and developers
    ...
    <p aria-roledescription="paragraph">
      ...
      <em>Erie</em>'s users may have limited understanding and skill with
      respect to acoustic engineering and audio programming, it makes more sense
      to be able to declare data-to-audio conversions with a few configuration
      terms. Consequently, we designed <em>Erie</em>'s syntax to express
      <em>data</em>
    ...
    <p aria-roledescription="paragraph">
      ...
      <em>Erie</em> allows for sampling audio files, configuring FM and AM
      synths, and defining periodic waves (combining multiple sine and cosine
      waves). Furthermore, <em>Erie</em> provides a method to define and connect
      custom audio filters (e.g., distortion, biquad filters) that can have extra
      auditory encoding channels.
    </p>
    <p aria-roledescription="paragraph">
      ...
      standards. First, <em>Erie</em>'s queue compiler generates a scheduled
      list of sound items using standard auditory units (e.g., Hz and musical
      notes for pitch, the panning range from <Eq math="1"></Eq> to <Eq
        math="-1"
      ></Eq>) so as to be used in other audio environments (e.g., external music
      software). Our <em>Erie</em> player for web employs the Web Audio and Speech
      APIs to enable cross-browser experience.
    </p>
  {/if}
</section>
